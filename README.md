...
**当前版本**：1.1.2（`versionCode` 4）· 包名 `com.familylibrary.app`

## 安装 / 更新 APK

### 若提示「开发者签名冲突 / 软件包冲突」

旧版 CI 包名为 **`com.familylibrary.app.debug`**，签名与新版不同，**不能覆盖安装**。

**推荐做法（不用卸载也能装）：**

1. 直接安装新版 APK（包名 `com.familylibrary.app`，与旧版不同）
2. 桌面会出现**第二个**「家庭图书馆」图标 → 用**新的**（设置里显示 `v1.1.2 (build 4)`）
3. 长按**旧的**图标 → 卸载（旧数据在旧包里，需要的话先在旧 App 里导出 ZIP）

**若仍无法安装：** 设置 → 应用 → 搜索「家庭图书馆」→ 全部卸载 → 再装新版。

| 核对项 | 正确的新版 |
|--------|-----------|
| 设置页版本 | `v1.1.2 (build 4)` |
| 包名 | `com.familylibrary.app` |
